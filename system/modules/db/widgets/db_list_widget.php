...
$this->add_form_field('quotes')->renderAs(frm_widget, array(
    'class'=>'Db_ListWidget', 
...
class Db_List_Widget extends Db_FormWidget
{
    ...
    protected function render_table()
    {
        ...
        if (!$this->custom_partial)
            $this->render_partial('list');
        else
            $this->render_partial($this->custom_partial);
    }
    ...
    private function init_settings()
    {
        ...
        if ($this->is_editable)
        {
            $this->record_onclick = "new PopupForm('".$this->controller->getEventHandler('onFormWidgetEvent')."', { ajaxFields: { 
                ".$this->get_event_handler_data('on_form_popup').", 
                primary_id: '%s', 
                edit_session_key: '".$this->controller->form_get_edit_session_key()."',
                form_context: '".$this->controller->formGetContext()."'
            } }); return false;";
        }
    }
    ...
    protected function prepare_render_data($no_pagination = false)
    {
        ...
        if (!$no_pagination && !$this->no_pagination)
        {
            ...
            $pagination->setRowCount($total_row_count);

            $pagination->setCurrentPageIndex($this->load_preference('page'));
        ...
        $sorting_field = $column_defintions[$sorting_column->field]->getSortingColumnName();
    ...
    public function cp_button($icon='add')
    {
        return cp_button('New '.$this->form_title, 'icon '.$icon, array(
            'onclick'=>"new PopupForm('".$this->controller->getEventHandler('onFormWidgetEvent')."', { 
                ajaxFields: {
                    ".$this->get_event_handler_data('on_form_popup').",
                    form_context: '".$this->controller->formGetContext()."', 
                    edit_session_key: '".$this->controller->form_get_edit_session_key()."'
    ...
    public function on_form_popup($field, $model)
    {
        ...
        try
        {
            ...
            if ($form_context == "preview") // Deferred sessions not needed
                $this->controller->resetFormEditSessionKey();
        ...
        {
            $this->controller->handlePageError($ex);
        }

        $this->render_partial('list_popup');
    }
    ...
    public function list_cell_class($column_definition)
    {
        ...
        $sorting_column = $this->eval_sorting_column();
        if ($sorting_column->field == $column_definition->dbName)
        {
            $result .= ' current ';
            $result .= $sorting_column->direction == 'asc' ? 'order_asc' : 'order_desc';
        }
    ...
    protected function eval_list_columns($only_visible = true)
    {
        ...
        else
        {
            ...
            foreach ($definitions as $column_name=>$definition)
            {
                ...
                && (($only_visible && $definitions[$column_name]->defaultVisible) || !$only_visible))
    ...
    protected function eval_sorting_column()
    {
        ...
        foreach ($definitions as $column_name=>$definition)
        {
            if ($definition->defaultOrder !== null)
                return (object)(array('field'=>$column_name, 'direction'=>$definition->defaultOrder));
        }
        ...
        $this->_sorting_column = (object)(array('field'=>$list_columns[$first_column]->dbName, 'direction'=>'asc'));
        return $this->_sorting_column;
    }    
