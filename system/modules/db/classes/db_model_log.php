...
class Db_Model_Log extends Phpr_Extension_Base
{
    ...
    public function __construct($model)
    {
        ...
        {
            $this->_model->add_event('onAfterCreate', $this, 'model_log_created');
            $this->_model->add_event('onAfterUpdate', $this, 'model_log_updated');
            $this->_model->add_event('onAfterDelete', $this, 'model_log_deleted');
        }

        $this->_model->add_event('onAfterLoad', $this, 'model_log_loaded');
    }
    ...
    {
        $params['message'] = $name;
        $xml_obj = Phpr_Xml::from_plain_array($params, 'record', true);
        $param_data = Phpr_Xml::beautify_xml($xml_obj);
    ...
    private function delete_old_records($number_to_keep = null)
    {
        ...
        $count = Db_Helper::scalar('select count(*) from model_log where '.$where, $bind);
        $offset = $count - $number_to_keep;

        if ($offset <= 0)
            return;

        Db_Helper::query('delete from model_log where '.$where.' order by record_datetime limit '.$offset, $bind);
    }
    ...
    private function get_display_values($model = null)
    {
        ...
        foreach ($fields as $db_name=>$definition)
        {
            ...
            $result[$db_name] = $model->displayField($db_name);
        }
